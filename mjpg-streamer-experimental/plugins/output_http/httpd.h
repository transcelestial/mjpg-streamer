...
/* the boundary is used for the M-JPEG stream, it separates the multipart stream
 * of pictures */
...
/*
 * this struct is used to hold information from the clients address, and last
 * picture take time
 */
...
/*
 * this struct is just defined to allow passing all necessary details to a
 * worker thread "cfd" is for connected/accepted filedescriptor
 */
typedef struct {
  context *pc;
  int fd;
