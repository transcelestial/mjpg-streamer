...
Input Value.: * fd.....: fildescriptor to read from
              * iobuf..: iobuffer that allows to use this functions from
multiple threads because the complete context is the iobuffer.
...
int _read(int fd, iobuffer *iobuf, void *buffer, size_t len, int timeout) {
  ...
  while ((copied < len)) {
     ...
     * But: It may happen (very seldomly), that the socket gets closed remotly
     * between the select() and the following read. That is the reason for not
     * relying on reading at least one byte.
     */
    ...
    /* align data to the end of the buffer if less than IO_BUFFER bytes were
     * read */
...
int unescape(char *string) {
  ...
  for (dst = 0, src = 0; src < length; src++) {
    ...
    if ((rc = hex_char_to_int(source[src + 1])) == -1)
      return -1;
    destination[dst] = rc * 16;
    if ((rc = hex_char_to_int(source[src + 2])) == -1)
      return -1;
    destination[dst] += rc;
...
Return Value: Returns with the newly added info or with a pointer to the
existing item
******************************************************************************/
client_info *add_client(char *address) {
  ...
  memset(&(current_client_info->last_take_time), 0,
         sizeof(struct timeval)); // set last time to zero

  client_infos.infos =
      realloc(client_infos.infos,
              (client_infos.client_count + 1) * sizeof(client_info *));
...
Input Value.: Client IP address as a string
Return Value: If a frame was served to it within the specified interval it
returns 1 If not it returns with 0
******************************************************************************/
int check_client_status(client_info *client) {
  ...
  for (; i < client_infos.client_count; i++) {
    if (client_infos.infos[i] == client) {
      ...
      msec +=
          (tim.tv_usec - client_infos.infos[i]->last_take_time.tv_usec) / 1000;
      DBG("diff: %ld\n", msec);
      if ((msec < 1000) && (msec > 0)) { // FIXME make it parameter
        DBG("CHEATER\n");
        pthread_mutex_unlock(&client_infos.mutex);
        return 0; // address
                  // https://github.com/jacksonliam/mjpg-streamer/issues/162
      } else {
        pthread_mutex_unlock(&client_infos.mutex);
        return 0;
      }
  ...
  DBG("Client not found in the client list! How did it happend?? This is a "
      "BUG\n");
  pthread_mutex_unlock(&client_infos.mutex);
  return 0;
}
...
void send_stream_wxp(cfd *context_fd, int input_number) {
  ...
  curDate = time(NULL);
  expiresDate = curDate - 1380; // teh expires date is before the current date
                                // with 23 minute (1380) sec

  char curDateBuffer[80];
  char expDateBuffer[80];

  strftime(curDateBuffer, 80, "%a, %d %b %Y %H:%M:%S %Z", localtime(&curDate));
  strftime(expDateBuffer, 80, "%a, %d %b %Y %H:%M:%S %Z",
           localtime(&expiresDate));
  sprintf(buffer,
          "HTTP/1.1 200 OK\r\n"
  ...
  while (!pglobal->stop) {
    ...
    DBG("sending intemdiate header\n");
    if (write(context_fd->fd, buffer, 50) < 0)
      break;
...
void execute_cgi(int id, int fd, char *parameter, char *query_string) {
  ...
  char *enviroment = "SERVER_SOFTWARE=\"mjpg-streamer\" "
                     //"SERVER_NAME=\"%s\" "
  ...
  buffer_length = 3;
  buffer_length =
      strlen(fn_buffer) + strlen(enviroment) + strlen(parameter) + 256;
  ...
  sprintf(buffer, enviroment, conf.port, parameter, query_string, fn_buffer);
...
void command(int id, int fd, char *parameter) {
  ...
  /* command format:
      ...
      where:
      dest: specifies the command destination (input, output, program itself)
     0-1-2 plugin specifies the plugin id  (not acceptable at the commands sent
     to the program itself) id: the control id group: the control's group eg.
     V4L2 control, jpg control, etc. This is optional value: value the control
  */
  ...
  if ((command = strstr(parameter, "id=")) == NULL) {
    ...
    send_error(fd, 400,
               "no GET variable \"id=...\" found, it is required to specify "
               "which command id to execute");
    return;
  }
  ...
  if ((value = strstr(parameter, "dest=")) != NULL) {
    ...
    switch (dest) {
    case Dest_Input:
      DBG("The command destination value converted form the string \"%s\" to "
          "integer %d -> INPUT\n",
          svalue, dest);
      break;
    case Dest_Output:
  ...
  if ((value = strstr(parameter, "plugin=")) != NULL) {
    ...
    DBG("The plugin number value converted value form string %s to integer "
        "%d\n",
        svalue, plugin_no);
  } else {
    value = NULL;
  ...
  case Dest_Input:
    if (plugin_no < pglobal->incnt) {
      ...
      DBG("Invalid plugin number: %d because only %d input plugins loaded",
          plugin_no, pglobal->incnt - 1);
    }
    break;
  case Dest_Output:
    if (plugin_no < pglobal->outcnt) {
      ...
      DBG("Invalid plugin number: %d because only %d output plugins loaded",
          plugin_no, pglobal->incnt - 1);
    }
    break;
  case Dest_Program:
...
void *client_thread(void *arg) {
  ...
  if (strstr(buffer, "GET /?action=snapshot") != NULL) {
...
#endif
#ifdef WXP_COMPAT
  } else if ((strstr(buffer, "GET /cam") != NULL) &&
             (strstr(buffer, ".jpg") != NULL)) {
    ...
    pb += strlen("GET /?action=take"); // a pb points to thestring after the
                                       // first & after command

    /* only accept certain characters */
    len = MIN(MAX(strspn(pb, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVW"
                             "XYZ_-=&1234567890%./"),
                  0),
              100);
    ...
    if (unescape(req.parameter) == -1) {
      free(req.parameter);
      send_error(lcfd.fd, 500,
                 "could not properly unescape command parameter string");
      LOG("could not properly unescape command parameter string\n");
      close(lcfd.fd);
      return NULL;
    }
  } else if ((strstr(buffer, "GET /input") != NULL) &&
             (strstr(buffer, ".json") != NULL)) {
    req.type = A_INPUT_JSON;
    query_suffixed = 255;
  } else if ((strstr(buffer, "GET /output") != NULL) &&
             (strstr(buffer, ".json") != NULL)) {
    ...
    pb += strlen("GET /?action=command"); // a pb points to thestring after the
                                          // first & after command
    ...
    if (strstr(pb, ".cgi") != NULL) {
      ...
      if (pb != NULL) {
        pb++; // skip the ?
        len = strspn(pb, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ."
                         "_-1234567890=&");
  ...
  if (query_suffixed) {
    ...
    if (sch != NULL) { // there is an _ in the url so the input number should be
                       // present
      ...
      if ((req.type == A_SNAPSHOT_WXP) ||
          (req.type ==
           A_STREAM_WXP)) { // webcamxp adds offset to the camera number
        input_number--;
      }
    }
    DBG("plugin_no: %d\n", input_number);
  ...
  case A_FILE:
    ...
    break;
  /*
      With the take argument we try to save the current image to file before we
     transmit it to the user. This is done trough the output_file plugin. If it
     not loaded, or the file could not be saved then we won't transmit the
     frame.
  */
  case A_TAKE: {
    int i, ret = 0, found = 0;
      ...
      if (pglobal->out[i].name != NULL) {
        if (strstr(pglobal->out[i].name, "FILE output plugin")) {
          ...
          if ((filename = strstr(req.parameter, "filename=")) != NULL) {
            ...
            DBG("Filename = %s\n", filenamearg);
            // int output_cmd(int plugin_id, unsigned int control_id, unsigned
            // int group, int value, char *valueStr)
            ret = pglobal->out[i].cmd(i, OUT_FILE_CMD_TAKE, IN_CMD_GENERIC, 0,
                                      filenamearg);
          } else {
            DBG("filename is not specified int the URL\n");
            send_error(
                lcfd.fd, 404,
                "The &filename= must present for the take command in the URL");
          }
          break;
        }
      }
    ...
    if (found == 0) {
      ...
      send_error(lcfd.fd, 404,
                 "FILE output plugin not loaded, taking snapshot not possible");
    } else {
      if (ret == 0) {
        send_snapshot(&lcfd, input_number);
      } else {
...
void *server_thread(void *arg) {
  ...
  for (aip2 = aip; aip2 != NULL; aip2 = aip2->ai_next) {
    ...
    on = 1;
    if (setsockopt(pcontext->sd[i], SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) <
        0) {
      perror("setsockopt(SO_REUSEADDR) failed\n");
    }

    /* IPv6 socket should listen to IPv6 only, otherwise we will get "socket
     * already in use" */
    on = 1;
    if (aip2->ai_family == AF_INET6 &&
        setsockopt(pcontext->sd[i], IPPROTO_IPV6, IPV6_V6ONLY,
  ...
  while (!pglobal->stop) {
    ...
    for (i = 0; i < max_fds + 1; i++) {
      if (pcontext->sd[i] != -1 && FD_ISSET(pcontext->sd[i], &selectfds)) {
        ...
        DBG("create thread to handle client that just established a "
            "connection\n");

        if (getnameinfo((struct sockaddr *)&client_addr, addr_len, name,
                        sizeof(name), NULL, 0, NI_NUMERICHOST) == 0) {
...
/******************************************************************************
Description.: Send a JSON file which is contains information about the input
plugin's acceptable parameters Input Value.: fildescriptor fd to send the answer
to Return Value: -
******************************************************************************/
void send_input_JSON(int fd, int input_number) {
  char buffer[BUFFER_SIZE * 16] = {
      0}; // FIXME do reallocation if the buffer size is small
  int i;
  sprintf(buffer,
          "HTTP/1.0 200 OK\r\n"
          "Content-type: %s\r\n" STD_HEADER "\r\n",
  ...
  if (pglobal->in[input_number].in_parameters != NULL) {
    for (i = 0; i < pglobal->in[input_number].parametercount; i++) {
      ...
      if (pglobal->in[input_number].in_parameters[i].ctrl.type ==
          V4L2_CTRL_TYPE_MENU) {
        if (pglobal->in[input_number].in_parameters[i].menuitems != NULL) {
          int j, k = 1;
          for (j = pglobal->in[input_number].in_parameters[i].ctrl.minimum;
               j <= pglobal->in[input_number].in_parameters[i].ctrl.maximum;
               j++) {
            char *tempName = NULL; // temporary storage for name sanity checking

            int prevSize = 0;
            int itemLength = strlen((char *)&pglobal->in[input_number]
                                        .in_parameters[i]
                                        .menuitems[j]
                                        .name);
            tempName = (char *)calloc(
                itemLength + 1,
                sizeof(char)); // allocate space for the sanity checking
            if (tempName == NULL) {
              DBG("Realloc/calloc failed: %s\n", strerror(errno));
              return;
            }

            check_JSON_string((char *)&pglobal->in[input_number]
                                  .in_parameters[i]
                                  .menuitems[j]
                                  .name,
                              tempName); // sanity check the string after non
            ...
            if (menuString == NULL) {
              ...
              menuString =
                  realloc(menuString, (strlen(menuString) + itemLength + 5) *
                                          (sizeof(char)));
            }
  ...
  if (pglobal->in[input_number].in_formats != NULL) {
    for (i = 0; i < pglobal->in[input_number].formatCount; i++) {
      ...
      int j = 0;
      for (j = 0; j < pglobal->in[input_number].in_formats[i].resolutionCount;
           j++) {
        char buffer_num[6];
        ...
        sprintf(buffer_num, "%d",
                pglobal->in[input_number]
                    .in_formats[i]
                    .supportedResolutions[j]
                    .width);
        resolutionsStringLength += strlen(buffer_num);
        sprintf(buffer_num, "%d",
                pglobal->in[input_number]
        ...
        resolutionsStringLength += strlen(buffer_num);
        if (j !=
            (pglobal->in[input_number].in_formats[i].resolutionCount - 1)) {
          resolutionsStringLength += (strlen("\"\": \"x\", ") + 5);
          if (resolutionsString == NULL)
            resolutionsString = calloc(resolutionsStringLength, sizeof(char *));
          else
            resolutionsString = realloc(
                resolutionsString, resolutionsStringLength * sizeof(char *));
          if (resolutionsString == NULL) {
      ...
      sprintf(buffer + strlen(buffer),
...
#ifdef V4L2_FMT_FLAG_COMPRESSED
              pglobal->in[input_number].in_formats[i].format.flags &
              ...
              pglobal->in[input_number].in_formats[i].format.flags &
                  ...
                  : "false",
#endif
              pglobal->in[input_number].in_formats[i].currentResolution != -1
                  ? "true"
                  : "false",
              resolutionsString);
...
void send_program_JSON(int fd) {
  ...
  for (k = 0; k < pglobal->incnt; k++) {
    sprintf(buffer + strlen(buffer),
            ...
            "\"args\": \"%s\"\n"
            "}",
            pglobal->in[k].param.id, pglobal->in[k].name, pglobal->in[k].plugin,
  ...
  sprintf(buffer + strlen(buffer),
          ...
          "]\n"*/
          "],\n");
  sprintf(buffer + strlen(buffer), "\"outputs\":[\n");
  for (k = 0; k < pglobal->outcnt; k++) {
    sprintf(buffer + strlen(buffer),
            ...
            "\"args\": \"%s\"\n"
            "}",
            pglobal->out[k].param.id, pglobal->out[k].name,
            pglobal->out[k].plugin, pglobal->out[k].param.parameters);
...
/******************************************************************************
Description.:   checks the source string for non printable characters and
replaces them with space the two arguments should be the same size allocated
memory areas Input Value.:   source Return Value:   destination
******************************************************************************/
...
/******************************************************************************
Description.: Send a JSON file which is contains information about the output
plugin's acceptable parameters Input Value.: fildescriptor fd to send the answer
to Return Value: -
******************************************************************************/
void send_output_JSON(int fd, int input_number) {
  ...
  if (pglobal->out[input_number].out_parameters != NULL) {
    for (i = 0; i < pglobal->out[input_number].parametercount; i++) {
      ...
      if (pglobal->out[input_number].out_parameters[i].ctrl.type ==
          V4L2_CTRL_TYPE_MENU) {
        if (pglobal->out[input_number].out_parameters[i].menuitems != NULL) {
          int j, k = 1;
          for (j = pglobal->out[input_number].out_parameters[i].ctrl.minimum;
               j <= pglobal->out[input_number].out_parameters[i].ctrl.maximum;
               j++) {
            int prevSize = strlen(menuString);
            int itemLength = strlen((char *)&pglobal->out[input_number]
                                        .out_parameters[i]
                                        .menuitems[j]
                                        .name) +
                             strlen("\"\": \"\"");
            if (menuString == NULL) {
              ...
              menuString =
                  realloc(menuString,
                          (strlen(menuString) + itemLength) * (sizeof(char)));
            }
            ...
            if (j !=
              ...
              sprintf(menuString + prevSize, "\"%d\": \"%s\", ", j,
                      (char *)&pglobal->out[input_number]
                          .out_parameters[i]
                          .menuitems[j]
                          .name);
            } else {
